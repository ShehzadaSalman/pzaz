...
const StudioProAI = () => {
  ...
  const aiCards = [
    { title: t("studio_pro.ai_card1_title"), description: t("studio_pro.ai_card1_desc"), icon: iconScriptAnalysis },
    { title: t("studio_pro.ai_card2_title"), description: t("studio_pro.ai_card2_desc"), icon: iconCharacterTracking },
    { title: t("studio_pro.ai_card3_title"), description: t("studio_pro.ai_card3_desc"), icon: iconSceneSuggestions },
    { title: t("studio_pro.ai_card4_title"), description: t("studio_pro.ai_card4_desc"), icon: iconAiResearch },
  ];
      ...
      <div className="container mx-auto px-6">
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-12 items-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
            <SectionHeader
              align="left"
              pillText={t("studio_pro.ai_pill")}
              pillClassName="bg-primary/10 text-primary"
              title={<>{t("studio_pro.ai_title")}{" "}<span className="font-lato font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-[#3207BC] to-[#409DFF]">{t("studio_pro.ai_title_highlight")}</span></>}
              description={t("studio_pro.ai_desc")}
              className="mb-8"
            />
            <div className="grid md:grid-cols-2 gap-4">
              {aiCards.map((card, index) => (
                <motion.div key={card.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: 0.1 * index }} className="bg-[#F7F2FD] rounded-[8px] p-4 flex gap-3">
