...
const BlogCard = ({
  ...
  return (
    <article className={isGrid ? "w-full" : "mx-auto w-full max-w-[980px]"}>
      <Link
        to={href}
        className="group block overflow-hidden rounded-[24px] border border-[#D8D9DE] bg-[#ECECEF] shadow-[0_18px_50px_rgba(18,18,30,0.08)] h-full"
      >
        ...
        <div className={`bg-[#ECECEF] ${isGrid ? "px-5 py-5" : "px-6 py-8 md:px-12 md:py-10"}`}>
          <div className={`${isGrid ? "mb-3" : "mb-6"} flex items-center justify-between gap-4`}>
            <div className="flex flex-wrap items-center gap-3 md:gap-5">
              <span className={`rounded-[10px] bg-[#D7D7DC] px-4 py-2 font-medium text-[#4F2BA6] ${isGrid ? "text-xs" : "text-sm"}`}>
                {category}
              </span>
              <span className={`leading-none text-[#A3A3A8] ${isGrid ? "text-lg" : "text-[28px]"}`}>·</span>
          ...
          <h2 className={`font-lato font-black leading-[1.05] text-[#4A4A4F] ${isGrid ? "mb-3 text-[20px] md:text-[22px]" : "mb-6 text-[48px] md:text-[60px]"}`}>
            {title}
          </h2>
