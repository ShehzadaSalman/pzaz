...
const FileSharingCraft = () => {
  ...
  const shifts = [
    { title: t("file_sharing.craft1_title"), description: t("file_sharing.craft1_desc") },
    { title: t("file_sharing.craft2_title"), description: t("file_sharing.craft2_desc") },
    { title: t("file_sharing.craft3_title"), description: t("file_sharing.craft3_desc") },
  ];

  return (
    <section className="section-padding bg-[#fbfbfb] relative overflow-hidden">
      <div className="max-w-6xl mx-auto px-6 relative z-10">
        <AnimatedSectionHeader wrapperClassName="text-center mb-16" pillText={t("file_sharing.craft_pill")} pillClassName="bg-primary/10 text-primary"
          title={<>{t("file_sharing.craft_title")}{" "}<span className="font-lato font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-[#3207BC] to-[#409DFF]">{t("file_sharing.craft_title_highlight")}</span></>}
          description={t("file_sharing.craft_desc")} />
        ...
        <motion.p initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.4 }} className="text-center text-lg text-muted-foreground max-w-2xl mx-auto">
          {t("file_sharing.craft_bottom_pre")}{" "}
          <span className="font-semibold text-foreground">{t("file_sharing.craft_bottom_highlight")}</span>
        </motion.p>
      </div>
    </section>
  );
