      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="max-w-5xl mx-auto text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            <Pill
              text="For Producers and Department Heads"
          ...
          >
            <p className="text-lg sm:text-xl text-muted-foreground leading-relaxed mb-3">
              Film productions are full of moving pieces. Departments preparing equipment, producers coordinating logistics, directors refining creative decisions, and editors preparing the next stage.
            </p>
            <p className="text-lg sm:text-xl text-foreground font-semibold leading-relaxed">
              Everyone is busy. Yet no one sees the full picture. And when that picture is unclear, production momentum slows.
            </p>
          </motion.div>
              ...
              <div className="bg-muted/50 rounded-xl aspect-[16/9] flex items-center justify-center relative overflow-hidden">
                <div className="absolute inset-4 bg-background rounded-lg shadow-inner flex flex-col">
                  <div className="flex items-center gap-2 border-b border-border/50 px-4 py-2">
                    ...
                    <span className="text-xs font-medium hidden lg:block">Task Board</span>
                    <div className="ml-auto flex gap-2">
                      {["To Do", "In Progress", "Review", "Done"].map((s, i) => (
                        <div key={i} className={`h-5 px-2 rounded text-[10px] flex items-center ${i === 1 ? "bg-primary/20 text-primary" : "bg-muted text-muted-foreground"}`}>
                  ...
                  <div className="flex-1 p-4 grid grid-cols-4 gap-3">
                    {[
                      { col: "To Do", tasks: ["Location scout", "Wardrobe prep"], color: "border-border/40 bg-muted/40" },
                      { col: "In Progress", tasks: ["Camera setup", "Script locked"], color: "border-primary/40 bg-primary/5" },
                      { col: "Review", tasks: ["Call sheets", "Shot list v2"], color: "border-amber-300/40 bg-amber-50/40" },
                      { col: "Done", tasks: ["Casting complete", "Budget approved"], color: "border-green-300/40 bg-green-50/40" },
                    ].map((col, i) => (
                      <div key={i} className="flex flex-col gap-2">
                        <p className="text-[9px] font-bold text-muted-foreground uppercase tracking-wide hidden lg:block">{col.col}</p>
                        {col.tasks.map((task) => (
                          <div key={task} className={`rounded-lg border-2 ${col.color} px-2 py-1.5 hidden lg:block`}>
                            <p className="text-[9px] text-foreground font-medium">{task}</p>
                          </div>
                        ))}
                      </div>
                    ))}
